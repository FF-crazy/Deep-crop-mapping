...
def plot_confusion_matrix(cm: np.ndarray, class_names: List[str], save_path: str = None, title: str = "Swin Transformer Confusion Matrix"):
    """
    绘制混淆矩阵
    """
...
def plot_training_history(history: Dict[str, List[float]], save_path: str = None, title: str = "Swin Transformer Training History"):
    """
    绘制训练历史
    """
...
def plot_attention_weights(
    ...
    if attention_weights.dim() == 4:
        ...
        attention_to_plot = attention_weights[:num_heads].cpu().numpy()
    
    fig, axes = plt.subplots(2, 2, figsize=(12, 12))
    fig.suptitle(f'Swin Transformer Attention Patterns (Layer {layer_idx})', fontsize=16)
    axes = axes.ravel()
...
def plot_window_attention(
    ...
    # 获取特征图
    with torch.no_grad():
        # 这里需要修改模型以返回中间特征和注意力权重
        features = model.patch_embed(input_tensor)[0]  # 获取patch embedding
        
    batch_size, num_patches, embed_dim = features.shape
    patch_resolution = int(np.sqrt(num_patches))
    
    # 重塑为2D特征图
    features_2d = features.view(batch_size, patch_resolution, patch_resolution, embed_dim)
    
    # 可视化不同窗口的特征
    fig, axes = plt.subplots(2, 3, figsize=(15, 10))
    fig.suptitle('Swin Transformer Window Features', fontsize=16)
    axes = axes.ravel()
    
    for i in range(6):
        if i < embed_dim:
            feature_map = features_2d[0, :, :, i].cpu().numpy()
...
def plot_class_performance(metrics: Dict[str, Any], save_path: str = None, title: str = "Swin Transformer Per-Class Performance"):
    """
    绘制每类性能图表 - Swin Transformer版本
    """
...
def visualize_predictions(
    ...
    num_samples: int = 4,
    save_path: str = None,
    title: str = "Swin Transformer Predictions"
):
    """
    ...
    for idx, i in enumerate(indices):
        ...
        axes[idx, 2].set_title('Swin Transformer Prediction')
        axes[idx, 2].axis('off')
...
if __name__ == "__main__":
    # 测试各个功能
    print("Swin Transformer Utils module loaded successfully!")
