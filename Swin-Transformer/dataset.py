#!/usr/bin/env python3
"""
数据加载和预处理模块 - Swin Transformer版本
处理多光谱时序数据的加载、预处理和批处理，适配Swin Transformer模型
"""
...
class CropMappingDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            scaler: 预训练的标准化器
            augment: 是否使用数据增强
            swin_patch_size: Swin Transformer的patch大小
        """
        self.x_data = x_data
        self.y_data = y_data
        ...
        # 确保patch_size能被swin_patch_size整除
        if self.patch_size % self.swin_patch_size != 0:
            print(f"Warning: patch_size ({self.patch_size}) is not divisible by swin_patch_size ({self.swin_patch_size})")
            print(f"Adjusting patch_size to {(self.patch_size // self.swin_patch_size + 1) * self.swin_patch_size}")
            self.patch_size = (self.patch_size // self.swin_patch_size + 1) * self.swin_patch_size
    ...
    def _apply_augmentation(self, x: torch.Tensor, y: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Swin特定增强：窗口级别的增强
        if torch.rand(1) < 0.3:
            x = self._apply_window_augmentation(x)
        
        # 时序增强：随机遮掩一些时间步
        if torch.rand(1) < 0.2:
            temporal_steps = x.shape[2]
            mask_ratio = 0.1
            num_mask = max(1, int(temporal_steps * mask_ratio))
    ...
    def _apply_window_augmentation(self, x: torch.Tensor) -> torch.Tensor:
        """
        应用窗口级别的增强，适配Swin Transformer的窗口注意力机制
        """
        H, W, T, C = x.shape
        window_size = self.swin_patch_size * 2  # 使用较大的窗口
            ...
            if torch.rand(1) < 0.5:
                ...
                for i in range(num_windows_h):
                    for j in range(num_windows_w):
                        ...
                        if torch.rand(1) < 0.3:
                            # 窗口内像素混洗
                            window_flat = window.view(-1, T, C)
                            perm_indices = torch.randperm(window_flat.shape[0])
                            window_shuffled = window_flat[perm_indices]
                            x[h_start:h_end, w_start:w_end] = window_shuffled.view(window_size, window_size, T, C)
        
        return x


class SwinDataAugmentation:
    """Swin Transformer专用数据增强类"""
    
    @staticmethod
    def window_based_mixup(x: torch.Tensor, alpha: float = 0.2) -> torch.Tensor:
        """
        基于窗口的Mixup增强
        """
        ...
        if height >= window_size and width >= window_size:
            for b in range(batch_size):
                ...
                for i in range(num_windows_h):
                    for j in range(num_windows_w):
                        if torch.rand(1) < 0.3:  # 30%的概率进行mixup
                            h_start = i * window_size
                            h_end = h_start + window_size
                            w_start = j * window_size
                            w_end = w_start + window_size
                            ...
                            # 获取另一个随机位置的窗口
                            rand_i = torch.randint(0, num_windows_h, (1,)).item()
                            rand_j = torch.randint(0, num_windows_w, (1,)).item()
                            rand_h_start = rand_i * window_size
                            rand_h_end = rand_h_start + window_size
                            rand_w_start = rand_j * window_size
                            rand_w_end = rand_w_start + window_size
                            
                            # Mixup
                            lam = np.random.beta(alpha, alpha)
                            window1 = x[b, h_start:h_end, w_start:w_end]
                            window2 = x[b, rand_h_start:rand_h_end, rand_w_start:rand_w_end]
                            
                            mixed_window = lam * window1 + (1 - lam) * window2
                            x[b, h_start:h_end, w_start:w_end] = mixed_window
        
        return x
    
    @staticmethod
    def shifted_window_augmentation(x: torch.Tensor, shift_ratio: float = 0.25) -> torch.Tensor:
        """
        移位窗口增强，模拟Swin Transformer的移位窗口机制
        """
        batch_size, height, width, temporal, spectral = x.shape
        
        for b in range(batch_size):
            if torch.rand(1) < 0.3:
                shift_h = int(height * shift_ratio * (torch.rand(1) - 0.5))
                shift_w = int(width * shift_ratio * (torch.rand(1) - 0.5))
                
                x[b] = torch.roll(x[b], shifts=(shift_h, shift_w), dims=(0, 1))
...
def prepare_data(
    ...
    info = {
        ...
        'scaler': full_dataset.scaler,
        'patch_size': full_dataset.patch_size,  # 使用调整后的patch_size
        'swin_patch_size': swin_patch_size,
        'input_shape': (full_dataset.patch_size, full_dataset.patch_size, x_data.shape[2], x_data.shape[3]),
...
def save_data_info(info: Dict[str, Any], save_path: str = "./Swin-Transformer/data_info.pkl"):
    """保存数据信息"""
...
def load_data_info(load_path: str = "./Swin-Transformer/data_info.pkl") -> Dict[str, Any]:
    """加载数据信息"""
...
if __name__ == "__main__":
    ...
    print(f"Adjusted patch size: {info['patch_size']}")
    print(f"Swin patch size: {info['swin_patch_size']}")
