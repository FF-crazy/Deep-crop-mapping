...
try:
    ...
    print("Warning: einops not found. Please install it with: pip install einops")
...
def train_model(config: Dict[str, Any]):
    ...
    train_loader, val_loader, test_loader, data_info = prepare_data(
        ...
        swin_patch_size=config.get('swin_patch_size', 4)
    )
    ...
    if config.get('use_wandb', False):
        wandb.init(project='crop-mapping-swin-transformer', config=config)
...
def main():
    parser = argparse.ArgumentParser(description='Train Swin Transformer model for crop mapping')
    ...
    # 模型参数
    parser.add_argument('--swin-patch-size', type=int, default=4, help='Swin patch size')
    ...
    parser.add_argument('--num-heads', type=int, nargs='+', default=[3, 6, 12, 24], help='Number of attention heads')
    parser.add_argument('--window-size', type=int, default=7, help='Window size for attention')
    parser.add_argument('--mlp-ratio', type=float, default=4.0, help='MLP ratio')
    parser.add_argument('--dropout', type=float, default=0.0, help='Dropout rate')
    parser.add_argument('--attn-dropout', type=float, default=0.0, help='Attention dropout rate')
    ...
    parser.add_argument('--max-grad-norm', type=float, default=5.0, help='Max gradient norm for clipping')
    ...
    # 其他参数
    parser.add_argument('--save-dir', type=str, default='./Swin-Transformer/checkpoints', 
