#!/usr/bin/env python3
"""
推理脚本 - Swin Transformer版本
使用训练好的Swin Transformer模型进行农作物制图预测
"""
...
class CropMappingSwinInference:
    """
    农作物制图Swin Transformer推理类
    """
    def __init__(self, model_path: str, device: str = 'cuda'):
        ...
        self.model = create_swin_model(
            ...
            patch_size=self.config.get('swin_patch_size', 4),
            ...
            drop_rate=self.config.get('dropout', 0.0),
            attn_drop_rate=self.config.get('attn_dropout', 0.0),
            drop_path_rate=0.0  # 推理时不使用drop_path
        ).to(self.device)
        ...
        print(f"Swin Transformer model loaded successfully on {self.device}")
        print(f"Number of classes: {self.data_info['num_classes']}")
        print(f"Model parameters: {sum(p.numel() for p in self.model.parameters()):,}")
        print(f"Window size: {self.config.get('window_size', 7)}")
    ...
    def predict_full_image(
        ...
        # 确保patch_size是swin_patch_size的倍数
        swin_patch_size = self.data_info.get('swin_patch_size', 4)
        if patch_size % swin_patch_size != 0:
            adjusted_patch_size = ((patch_size // swin_patch_size) + 1) * swin_patch_size
            print(f"Adjusting patch_size from {patch_size} to {adjusted_patch_size} to match Swin patch size")
        ...
        print(f"Predicting {len(patches)} patches with Swin Transformer...")
    ...
    def _predict_with_tta(self, batch_patches: torch.Tensor) -> np.ndarray:
        """
        使用测试时间增强进行预测 - Swin Transformer版本
        """
        all_probs = []
        ...
        with torch.no_grad():
            ...
            # 对于Swin Transformer，窗口大小的旋转可能会影响性能，因此只使用翻转
    ...
    def visualize_results(
        ...
        ax1.set_title('Swin Transformer Crop Type Predictions', fontsize=16)
        ax1.axis('off')
...
def main():
    parser = argparse.ArgumentParser(description='Run inference with Swin Transformer model')
    ...
    parser.add_argument('--output-dir', type=str, default='./Swin-Transformer/predictions', help='Output directory')
    ...
    # 初始化推理器
    inferencer = CropMappingSwinInference(args.model_path)
    ...
    print(f"Swin Transformer inference completed in {inference_time:.2f} seconds")
    ...
    if args.visualize:
        inferencer.visualize_results(
            predictions,
            confidence_map,
            save_path=output_dir / 'swin_predictions_visualization.png',
    ...
    print(f"\nSwin Transformer inference results saved to {output_dir}")
